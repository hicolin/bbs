...
class AdminCountController extends BaseController
{
    ...
    */
    public function actionCommission3($pid, $money, $grade, $num)
    {
        ...
        $rate = explode(',', $product->fy_one);    // 1级 钻石
        $rate2 = explode(',', $product->fy_two);   // 2级 金牌
        $rate3 = explode(',', $product->fy_three); // 3级 普通
        if ($grade != 0) {
            if ($grade == 3) { //  钻石 一 二 三 级
                foreach ($rate as $key => $value) {
                    if ($key == ($num - 1)) {
