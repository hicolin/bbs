...
class AdminPrivateMessageController extends BaseController
{
    ...
    {
        $query = AdminPrivateMessage::find()->joinWith('member')->joinWith('user');
    ...
    public function condition($query, $search)
    {
        ...
        if (isset($search['content']) && $search['content']) {
            $query->andWhere(['like', 'admin_private_message.content', $search['content']]);
        }
        if (isset($search['status']) && $search['status']) {
            $query->andWhere(['=', 'admin_private_message.status', $search['status']]);
        }
        if (isset($search['b_time']) && $search['b_time']) {
            $query->andWhere(['>', 'admin_private_message.create_time', $search['b_time']]);
        }
        if (isset($search['e_time']) && $search['e_time']) {
            $query->andWhere(['<', 'admin_private_message.create_time', $search['e_time']]);
        }
        return $query;
    }
    ...
    public function actionChangeStatus()
    {
        if (Yii::$app->request->isPost) {
            ...
            $model = AdminPrivateMessage::findOne($post['id']);
            $model->status = $status;
            if (!$model->save(false)) {
    ...
    public function actionDel()
    {
        ...
        $model = AdminPrivateMessage::findOne($id);
        if (!$model->delete()){
    ...
    public function actionBatchDel()
    {
        ...
        $res = AdminPrivateMessage::deleteAll(['in', 'id', $ids]);
        if (!$res) {
