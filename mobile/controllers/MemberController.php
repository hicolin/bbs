...
class MemberController extends BaseController
{
    ...
    public function actionIndex()
    {
        ...
        $privateMessage = AdminPrivateMessage::find()->where(['to_user_id' => $userId, 'is_read' => 1])->one();
        $privateMessage && $hasPrivateMessage = true;
        // 检测新回复
        $hasReply = false;
        $reply = AdminReply::find()->where(['to_user_id' => $userId, 'is_read' => 1])->one();
        $reply && $hasReply = true;
        return $this->render('index',compact('user_info', 'hasPrivateMessage', 'hasReply'));
    }
...
//                ->andWhere(['between','create_time',$beginTime,$endTime])->sum('money');
    ...
    public function createPosters($poster_path, $url, $pid='', $filename_code='')
    {
        ...
        $path = './uploads/img/qrcode/';
        is_dir($path) || mkdir($path, 0755, true);
        $filename_code = $filename_code ?: $path . '_promote.jpg';
    ...
    public function createPoster($poster_path, $url, $pid='', $filename_code='')
    {
        ...
        is_dir($qrcode_path) or mkdir($qrcode_path,0777,true);
         ...
         $path = './uploads/img/qrcode/';
          is_dir($path) || mkdir($path, 0755, true);
        $filename_code = $filename_code ?: $path .$code_num.'_promote.jpg';
        PublicController::qrCode($url,$filename_code,10);
        //创建图片的实例
        //var_dump($poster_path);exit;
        $dst = imagecreatefromstring(file_get_contents($poster_path));
...
//        $end_month=mktime(23,59,59,date('m'),date('t'),date('Y'));
    ...
    public function actionArticle()
    {
        ...
        if (Yii::$app->request->isAjax) {
            $cat_ids = Yii::$app->request->post('cat_ids');
            ...
            if ($pullType == 'up') {
                ...
                if ($keywords) {
                    $query = $query->andWhere(['like', 'title', $keywords]);
                } else {
                    if (!in_array($cat_ids, range(4, 7))) {
                        $cat_ids = 4;
                    }
                    $query = $query->andWhere(['cat_id' => $cat_ids]);
                }
                $total = $query->count();
                $data = $query->orderBy('create_time desc')
                    ->offset($offset)->limit($pageSize)
                    ->asArray()->all();
                $data = AdminArticle::mFormatData($data);
                return $this->json(200, '获取成功', compact('total', 'data'));
            } else {
                $requestTime = Yii::$app->request->get('request_time');
                $data = AdminArticle::find()
                    ->where(['art_id' => $cat_ids])
        ...
        $banners = AdminBanner::find()->where(['type' => 4])->orderBy('create_time desc')
            ->limit(5)->all();
        $category = AdminCategory::find()->orderBy('id asc')->limit(4)->all();
        $data = compact('category', 'keywords', 'user_info', 'banners', 'cat_ids');
