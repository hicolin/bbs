...
class IndexController extends MobileController
{
    ...
    public function actionIndex()
    {
        ...
        $banners = AdminBanner::find()->where(['type' => 1])->all();
...
// 注册操作
    public function actionRegisteradd()
    {
        if (Yii::$app->request->isPost){
            ...
            if ($yqm && !$invitations) {
                Yii::$app->getSession()->setFlash('error', '您输入邀请码不正确');
            ...
            if (($code_num != $code) || ($code_tel != $tel)) {
                Yii::$app->getSession()->setFlash('error', '注册失败,手机号或验证码不正确');
                echo "<script>window.history.go(-1)</script>";
                exit;
            } elseif ($count > 0) {
                Yii::$app->getSession()->setFlash('error', '注册失败,手机号码已经注册');
                echo "<script>window.history.go(-1)</script>";
                exit;
            } else {
                /**
...
//                    $url = Yii::$app->urlManager->createAbsoluteUrl('index/login');
    ...
    public function actionLoginout()
    {
        ...
        $url = Yii::$app->urlManager->createAbsoluteUrl(['post/index']);
        return $this->redirect($url);
    }
