...
class ListController extends BaseController
{
    ...
    public function actionBuyAgent()
    {
        ...
        $comments = AdminCommemts::find()->joinWith('member')
            ...
            ->offset(0)
            ->limit(2)->all();
        $goldPrice = AdminGrade::findOne(2)->price;
        $diamondPrice = AdminGrade::findOne(3)->price;
        if ($user_info->grade == 2) {
            $diamondPrice = $diamondPrice - $goldPrice;
        }
        return $this->render('buy-agent',compact('goldPrice', 'diamondPrice','comments',
            'user_info','poster','webTitle'));
    }
...
//            }
            ...
            $grade = (int)Yii::$app->request->post('grade');
        ...
        if (!in_array($grade, [2, 3])) {
            return '参数错误';
        }
        $user_info = AdminMember::findOne($user_id);
        $price = AdminGrade::findOne($grade)->price;
        if ($user_info->grade == 2 && $grade == 3) {
            $goldPrice = AdminGrade::findOne(2)->price;
            $price -= $goldPrice;
        }
//        if ($user_id == 9290) {
...
//        }
        ...
        return $this->render('sub-data',compact('user_info', 'grade','price','pay_ways'));
    }
