...
<div class="pay_main">
    ...
    <ul class="payOrder_list">
        <li>
            <h1>商品名称</h1>
            <p><?= \mobile\controllers\Service::getMemberType($grade) ?></p>
        </li>
        <li>
            <h1>商家名称</h1>
...
<script>
    ...
    function data_check() {
        ...
        var grade = '<?= $grade ?>';
        layer.load(2);
        $.post('<?= Url::to(['list/sub-data'])?>',{_csrf:_csrf, type: type, price: price, grade: grade},function (res) {
            layer.closeAll();
            if(res.status === 200){
                if(type == 1){  // 支付宝支付
                    if(isWeiXin()){
                        layer.msg('由于微信内不能使用支付宝支付，请使用浏览器支付');
                        return false;
                    }
                    url = res.url;
                }
